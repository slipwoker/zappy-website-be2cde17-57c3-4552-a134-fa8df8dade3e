  ...
  const dynamicObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        ...
        const nested = node.querySelectorAll ? node.querySelectorAll('.' + OBSERVE_CLASS) : [];
        nested.forEach(function (el) {
          if (!el.classList.contains(ANIMATION_CLASS)) {
            if ('IntersectionObserver' in window) {
              const nestedObserver = new IntersectionObserver(function (entries) {
                entries.forEach(function (entry) {
                  if (entry.isIntersecting) {
                    entry.target.classList.add(ANIMATION_CLASS);
                    nestedObserver.unobserve(entry.target);
                  }
                });
              }, { threshold: THRESHOLD, rootMargin: ROOT_MARGIN });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "השטיח הנקי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'be2cde17-57c3-4552-a134-fa8df8dade3e',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'be2cde17-57c3-4552-a134-fa8df8dade3e';
            ...
            if (mobileToggle && navMenu) {
                ...
                if (phoneHeaderBtn) {
                    phoneHeaderBtn.addEventListener('click', function() {
                        ...
                        // Falls back to 0545750481 placeholder which businessInfoUpdater can replace
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'be2cde17-57c3-4552-a134-fa8df8dade3e';
